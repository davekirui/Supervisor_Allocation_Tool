...
class xcoder_auth extends CI_Controller {
    ...
    {  
       ...
       // $data=  $this->post->xss_clean($data);//filter feed data 
            ...
            $this->email->message('Click the link to activate your account'.anchor('http://localhost/CS/index.php/xcoder_auth/conf/'.$xcoder_passkey));
           echo "Sending Mail...";
           try 
            {
               $this->email->send();
    ...
    {
        $criteria2=array($this->input->post('reg_no'),
        ...
        if($xcoder_count<=0)
        {
            $data['msg']="Oops! No Such Account Exists!!!";
            $data['page_title']="Wrong user Credentials";
           $this->load->view('wrong_login',$data);
          
        }        
        else if($xcoder_count>1)
        {
            $data['msg']="Oops Account Suspended because you share an account with someone else";
            $data['page_title']="Wrong Login";
           $this->load->view('wrong_login',$data);           
        }
        else
        {
            ...
            $sess_array=array('email'=>$sess_data->xemail,
                              ...
                              'reg_no'=>$sess_data->reg_no,
                              'name'=>$sess_data->name,
                              'course'=>$sess_data->course,
                              'phone'=>$sess_data->phone,
                              'intrests'=>$sess_data->intrests,
                              'logged_in'=>true               
                    );
            }
