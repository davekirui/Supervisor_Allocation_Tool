...
echo form_open('Supervisors/feed') ?>
        <fieldset style=" border-color:#617798; border-style:solid; border-radius:6px">
        <legend style="color:#617798; font-weight:bold">Select Project Specifics</legend>
          <pre>
 ...
 <select name="project">
	<?php while($row1=mysql_fetch_array($sql1)) {?>  
	 ...
	 <?php }?>
	    ...
	    <br />
        <fieldset style=" border-color:#617798; border-style:solid; border-radius:6px">
        <legend style="color:#617798; font-weight:bold">Select Supervisors Available</legend>
          <?php
   while($row=mysql_fetch_array($sql))
   {
   echo "<input type='checkbox' name='value[]' value='".$row['sur_name']."'/>".$row['sur_name']."<br>";
   }
   
   ?>
        </fieldset>
	    <p class="submit">
          <input name="submit" type="submit" value="Complete Assignment" />
        </p>
